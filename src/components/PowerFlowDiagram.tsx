  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.diagram}>
        ...
        {/* Overlay labels */}
        <View style={[styles.nodeLabel, { left: nodes.grid.x - 14, top: nodes.grid.y + 22 }]}>
          <Text style={styles.nodeLabelText}>{gridOn ? '🔌' : '❌'}</Text>
          <Text style={styles.nodeLabelTitle}>Grid</Text>
        </View>
        <View style={[styles.nodeLabel, { left: nodes.solar.x - 14, top: nodes.solar.y + 20 }]}>
          <Text style={styles.nodeLabelText}>☀️</Text>
          <Text style={styles.nodeLabelTitle}>{pvPower > 0 ? `${pvPower}W` : 'Solar'}</Text>
        </View>
        <View style={[styles.nodeLabel, { left: nodes.inverter.x - 20, top: nodes.inverter.y + 24 }]}>
          <Text style={styles.nodeLabelTitle}>⚡ INV</Text>
        </View>
        <View style={[styles.nodeLabel, { left: nodes.battery.x - 14, top: nodes.battery.y + 20 }]}>
          <Text style={styles.nodeLabelText}>🔋</Text>
          <Text style={styles.nodeLabelTitle}>{batterySoc}%</Text>
        </View>
        <View style={[styles.nodeLabel, { left: nodes.house.x - 14, top: nodes.house.y + 22 }]}>
          <Text style={styles.nodeLabelText}>🏠</Text>
        ...
        {gridOn && (
          <FlowDot
            ...
            x1={nodes.grid.x + 18 - 140} y1={nodes.grid.y - 100}
            x2={nodes.inverter.x - 18 - 140} y2={nodes.inverter.y - 100}
            delay={0}
          />
        )}
        <FlowDot
          color={Colors.amber}
          x1={nodes.inverter.x + 18 - 140} y1={nodes.inverter.y - 100}
          x2={nodes.house.x - 18 - 140} y2={nodes.house.y - 100}
          delay={300}
        />
        {isDischarging && (
          <FlowDot
            color={Colors.amberLight}
            x1={nodes.battery.x - 140} y1={nodes.battery.y - 16 - 100}
            x2={nodes.inverter.x - 140} y2={nodes.inverter.y + 18 - 100}
            delay={150}
          />
        )}
        {isCharging && (
          <FlowDot
            color={Colors.blue}
            x1={nodes.inverter.x - 140} y1={nodes.inverter.y + 18 - 100}
            x2={nodes.battery.x - 140} y2={nodes.battery.y - 16 - 100}
            delay={150}
          />
        )}
      </View>
